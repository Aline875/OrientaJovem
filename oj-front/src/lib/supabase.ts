...
export async function cadastrarJovem(dados: CadastroJovem) {
  try {
    ...
    const { data: emailCheck, error: emailError } = await supabase
      ...
      .or(
        `email.eq.${dados.email.toLowerCase().trim()},login.eq.${dados.login.toLowerCase().trim()}`
      );
    ...
    const dadosLimpos = {
      ...
      ...(dados.cpf &&
        dados.cpf.trim() && { cpf: dados.cpf.replace(/\D/g, "") }),
    };
...
export async function testarConexao() {
  try {
    const { error } = await supabase.from("jovem").select("count").limit(1);
